        ...
        for next_hop in next_hop_list:
            for result in neigb_relations_results:
                ...
                if (switch_identifier_local == next_hop):
                    if_name_local = result["name"].encode("ascii","ignore")
                    switch_identifier_remote = switch_identifier_by_phy_address.get(str(''.join(chr(i) for i in result["switchidentifierfk"]).replace(":", "")))
                    if(switch_identifier_remote==router_id):
                        ...
                        for result in interfaces:
                            ...
                            if_name = result["name"].encode("ascii","ignore")
...
################################6
        ...
        print("----------------------------------")
        print("----------------------------------")
        for i in final_router_network_destination_view_dic:
            print("-" + i)
            for network in final_router_network_destination_view_dic[i]:
                print("|")
                print("|")

                print("----->" + network )
                for nexthop in final_router_network_destination_view_dic[i][network]:
                    print("|")
                    print("|")
        ...
        for router_id in final_router_network_destination_view_dic:
            t = PrettyTable(["Switch"])
            t.add_row([router_id])
            print (t)

            t = PrettyTable(["Networks","Reached by switches","Next-hop","IP route"])
            for network in final_router_network_destination_view_dic[router_id]:
                t.add_row([network, router_network_destination_view_dic[router_id][network],final_router_network_destination_view_dic[router_id][network],
				Routing.getNextHopIPPrint(router_id,network,prefix_lenght,final_router_network_destination_view_dic[router_id][network],neigb_relations_results,interfaces,switch_identifier_by_phy_address)])
            print(t)
        """
    ...
    def convergance(db_operations):
        ...
        for db_route in db_route_list:
            ...
            next_hop =  ''.join(chr(i) for i in db_route["mexthop"])
            ...
            for route in route_list[:]:
                if (route.router_id==switch_identifier_fk
                    ...
                    print("Route removed from the list: " + route_prefix + "-> " + next_hop + " " + route.router_id)
                    break
            ...
            if (route_exists==False):
                ...
                print("Route removed from the database: " + route_prefix + "-> "+next_hop)

        # Go on with the rest operations

        for route in route_list:
            print("Route to be insert on the database: " + route.network + "-> " + route.next_hop)
...
"""
@staticmethod
def getNextHopIPPrint(router_id, network, prefix_lenght, next_hop_list, neigb_relations_results, interfaces,
                      switch_identifier_by_phy_address):
    # get the IP address of the interface that relates to this router and is listed on the next-hop list
    ...
    for next_hop in next_hop_list:
        for result in neigb_relations_results:
            ...
            if (switch_identifier_local == next_hop):
                ...
                switch_identifier_remote = switch_identifier_by_phy_address.get(str(result[1]).replace(":", ""))
                if (switch_identifier_remote == router_id):
                    # Get the interface IP address!
                    for result in interfaces:
                        ip = result[0]
                        ...
                        if_name = result[3]
                        if (if_name_local == if_name and switch_id == next_hop):
                            exit_string = exit_string + " ip route add " + network + "/" + str(
                                prefix_lenght) + " via " + str(ip)
